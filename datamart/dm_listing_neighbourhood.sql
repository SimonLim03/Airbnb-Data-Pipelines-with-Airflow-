...
--FORMAT(date, 'MMMM/yyyy')::date AS month_year,
...
((COUNT(CASE WHEN has_availability = 't' THEN 1 END) - LAG(COUNT(CASE WHEN has_availability = 't' THEN 1 END)) OVER (PARTITION BY listing_neighbourhood ORDER BY CONCAT(EXTRACT(MONTH FROM date), '/', EXTRACT(YEAR FROM date)))) / NULLIF(LAG(COUNT(CASE WHEN has_availability = 't' THEN 1 END)) OVER (PARTITION BY listing_neighbourhood ORDER BY CONCAT(EXTRACT(MONTH FROM date), '/', EXTRACT(YEAR FROM date))), 0)) * 100 AS percentage_change_active_listings,
((COUNT(CASE WHEN has_availability = 'f' THEN 1 END) - LAG(COUNT(CASE WHEN has_availability = 'f' THEN 1 END)) OVER (PARTITION BY listing_neighbourhood ORDER BY CONCAT(EXTRACT(MONTH FROM date), '/', EXTRACT(YEAR FROM date)))) / NULLIF(LAG(COUNT(CASE WHEN has_availability = 'f' THEN 1 END)) OVER (PARTITION BY listing_neighbourhood ORDER BY CONCAT(EXTRACT(MONTH FROM date), '/', EXTRACT(YEAR FROM date))), 0)) * 100 AS percentage_change_inactive_listings,
